...
class CMM3App(tk.Tk):
    ...
    def perform_analysis(self, threshold, cancel_event):
        """
        Main analysis routine run in a separate thread based on input parameters.
        Handles logging, progress, and results.
        """
    ...
    def handle_completion(self, result):
        ...
        if status == "error":
            ...
            try:
                ...
                ax3.plot(critical_current, critical_y, 'ro', markersize=10, label=f'Critical: {critical_current:.1f} A', zorder=6)
