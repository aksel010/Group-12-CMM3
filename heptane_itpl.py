import numpy as np
import pandas as pd
from scipy.interpolate import interp1d
import matplotlib.pyplot as plt
from pathlib import Path
from typing import Any, cast

# --- Constants ---
N_HEPTANE_MOLAR_MASS = 0.100205  # kg/mol
DITTUS_BOELTER_EXPONENT = 0.4   # for heating
NIST_DATA_FILE = Path(__file__).parent / 'n heptane 2.txt'

# fixed parameters for the cooling channel
M_DOT = 0.05    # Mass flow rate [kg/s]
D = 0.005       # Channel diameter [m]

#clean NIST data
def import_nist_txt(filepath: str | Path) -> pd.DataFrame:
    """
    Imports and cleans fluid property data from a NIST text file.
    """
    df = pd.read_csv(filepath, sep=r'\s+', skiprows=1, header=None)
    
    column_map = {
        0: 'T_K',          
        1: 'Pressure',
        2: 'Density',         
        8: 'Cp',              
        12: 'Thermal_Cond',    
        11: 'Viscosity'        
    }

    df = df.rename(columns=column_map)[list(column_map.values())]
    
    # Unit conversions to SI
    df['Density'] = df['Density'] * 1000 * N_HEPTANE_MOLAR_MASS  # mol/L to kg/m^3
    df['Cp'] = df['Cp'] / N_HEPTANE_MOLAR_MASS                  # J/(mol·K) to J/(kg·K)
    df['Viscosity'] = df['Viscosity'] * 1e-6 
    
    return df

#import and assignment
nist_df = import_nist_txt(NIST_DATA_FILE)

T_data = nist_df['T_K'].values
rho_data = nist_df['Density'].values
Cp_data = nist_df['Cp'].values
lambda_data = nist_df['Thermal_Cond'].values
mu_data = nist_df['Viscosity'].values

# derived constant C_Re
C_RE = 4 * M_DOT / (np.pi * D)
#interplate

lambda_func = interp1d(T_data, lambda_data, kind='cubic', bounds_error=False, fill_value=cast(Any, (lambda_data[0], lambda_data[-1])))
Cp_func = interp1d(T_data, Cp_data, kind='cubic', bounds_error=False, fill_value=cast(Any, (Cp_data[0], Cp_data[-1])))
mu_func = interp1d(T_data, mu_data, kind='cubic', bounds_error=False, fill_value=cast(Any, (mu_data[0], mu_data[-1])))
rho_func = interp1d(T_data, rho_data, kind='cubic', bounds_error=False, fill_value=cast(Any, (rho_data[0], rho_data[-1])))

#create H(T)
def calculate_h(T: float | np.ndarray) -> float | np.ndarray:
    """
    Calculates the heat transfer coefficient (h) for n-heptane at a given temperature.
    This function is vectorized to accept NumPy arrays.
    """
    lam = lambda_func(T)
    Cp = Cp_func(T)
    mu = mu_func(T)
    
    # Use Dittus-Boelter equation for Nusselt number
    Re = C_RE / mu
    Pr = (Cp * mu) / lam
    Nu = 0.023 * (Re**0.8) * (Pr**DITTUS_BOELTER_EXPONENT)
    h = (Nu * lam) / D
    
    return h

#AI plotting (ommitted from script calling)
if __name__ == '__main__':

    def debug_calculation(T: float):
        """Prints a detailed breakdown of the heat transfer coefficient calculation."""
        print(f"\\n--- Debugging Calculation at T = {T:.1f} K ---")
        
        mu = mu_func(T)
        lam = lambda_func(T)
        Cp = Cp_func(T)
        Re = C_RE / mu
        Pr = (Cp * mu) / lam
        Nu = 0.023 * (Re**0.8) * (Pr**DITTUS_BOELTER_EXPONENT)
        h = (Nu * lam) / D
        
        print(f"  - Dynamic Viscosity (mu)    : {mu:.6f} Pa·s")
        print(f"  - Thermal Conductivity (lam): {lam:.4f} W/(m·K)")
        print(f"  - Specific Heat (Cp)        : {Cp:.2f} J/(kg·K)")
        print(f"  - Reynolds Number (Re)      : {Re:.0f}")
        print(f"  - Prandtl Number (Pr)       : {Pr:.2f}")
        print(f"  - Nusselt Number (Nu)       : {Nu:.2f}")
        print(f"  - Heat Transfer Coeff (h)   : {h:.2f} W/(m²·K)")

    # --- Setup Data for Plots ---
    print(f"System Constant C_Re = {C_RE:.2f} kg/(m·s)\\n")
    test_T = np.linspace(T_data.min(), T_data.max(), 200)
    
    h_values = calculate_h(test_T)
    mu_values = mu_func(test_T)
    Cp_values = Cp_func(test_T)
    lambda_values = lambda_func(test_T)
    
    # --- Plotting ---
    plt.style.use('seaborn-v0_8-darkgrid')
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 12))
    
    # --- Subplot 1: Fluid Properties ---
    p1, = ax1.plot(test_T, mu_values, label=r'Viscosity ($\mu$)', color='blue')
    ax1.set_xlabel('Temperature (T) [K]')
    ax1.set_ylabel(r'Viscosity ($\mu$) [Pa·s]', color=p1.get_color())
    ax1.tick_params(axis='y', labelcolor=p1.get_color())
    
    ax1_twin = ax1.twinx()
    p2, = ax1_twin.plot(test_T, Cp_values, label=r'Specific Heat ($C_p$)', color='green', linestyle='--')
    ax1_twin.set_ylabel('Specific Heat ($C_p$) [J/(kg·K)]', color=p2.get_color())
    ax1_twin.tick_params(axis='y', labelcolor=p2.get_color())

    ax1_twin2 = ax1.twinx()
    ax1_twin2.spines['right'].set_position(('outward', 60))
    p3, = ax1_twin2.plot(test_T, lambda_values, label=r'Thermal Conductivity ($\lambda$)', color='red', linestyle=':')
    ax1_twin2.set_ylabel(r'Thermal Conductivity ($\lambda$) [W/(m·K)]', color=p3.get_color())
    ax1_twin2.tick_params(axis='y', labelcolor=p3.get_color())

    ax1.set_title('Interpolated Fluid Properties of n-Heptane vs. Temperature')
    ax1.grid(True, linestyle='--', alpha=0.6)
    ax1.legend(handles=[p1, p2, p3])

    # --- Subplot 2: Heat Transfer Coefficient (h) ---
    T_test = 330.0
    h_test = calculate_h(T_test)
    
    ax2.plot(test_T, h_values, label=rf'h(T) at $\dot{{m}}$={M_DOT} kg/s, D={D*1000:.0f} mm', color='teal')
    ax2.plot(T_test, h_test, 'o', markersize=8, color='red', label=f'Test Point ({T_test} K, {h_test:.0f} W/m²K)')
    
    ax2.set_title('Heat Transfer Coefficient vs. Temperature for Liquid n-Heptane')
    ax2.set_xlabel('Temperature (T) [K]')
    ax2.set_ylabel('Heat Transfer Coefficient (h) [W/(m²·K)]')
    ax2.grid(True, linestyle='--', alpha=0.6)
    ax2.legend()
    
    plt.tight_layout(pad=3.0)
    plt.show()

    # --- Detailed Calculation for a Test Point ---
    debug_calculation(T_test)
    debug_calculation(350.0)
